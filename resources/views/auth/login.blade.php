...
@include('mainpartials.nav')
        ...
        <form class="loginContainer" method="POST" action="{{ route('login') }}">
            @csrf
            <span class="loginM1">Login to NFTs</span>
            ...
            <input class="loginInput" type="text" placeholder="Your email address" name="email" :value="old('email')" required autofocus>
            <input class="loginInput" type="password" placeholder="Type your password" type="password"
                   name="password"
            ...
            <div class="loginOptions">
                <label class="rememberMe">
                    <input class="checkboxLogin" type="checkbox" id="remember_me" name="remember" >Remember me
                </label>
                @if (Route::has('password.request'))
                <a class="ForgotPass" href="{{ route('password.request') }}">Forgot Password ?</a>
